...
class DeviceTransactionsDA {
        ...
        .aggregate([
          ...
          },
          {
            $group: {
              ...
              transactions: {
                $sum: { $cond: [{ $eq: ["$success", true] }, "$value", 0] }
              },
              errors: {
                $sum: { $cond: [{ $eq: ["$success", false] }, "$value", 0] }
              }
            }
          },
          {
  ...
  static getDeviceTransactionGroupByGroupName$(evt) {
    return Rx.Observable.forkJoin(
      DeviceTransactionsDA.getDeviceTransactionGroupByGroupNameInInterval$(
        evt.timeRanges[0],
        evt.endTimeLimit,
        evt.timeRangesLabel[0]
      ),
      DeviceTransactionsDA.getDeviceTransactionGroupByGroupNameInInterval$(
        evt.timeRanges[1],
        evt.endTimeLimit,
        evt.timeRangesLabel[1]
      ),
      DeviceTransactionsDA.getDeviceTransactionGroupByGroupNameInInterval$(
        evt.timeRanges[2],
        evt.endTimeLimit,
        evt.timeRangesLabel[2]
      )
    );
  }

  static getDeviceTransactionGroupByGroupNameInInterval$(
    startDate,
    endTime,
    timeInterval
  ) {
    ...
    return Rx.Observable.defer(() =>
      collection
        .aggregate([
          {
            $match: {
              timestamp: { $gte: startDate, $lte: endTime },
              success: true
            }
          },
          {
